...
if($db->connect_errno==0)
{
    ...
    if($pesansalah == "")
    {
        ...
        $sql = "INSERT INTO penggajian 
                VALUES('$no_slip','$periode_gaji','$tanggal','$kode_karyawan','$gaji_pokok','$tunjangan_bagian','$total_lembur','$total_gaji','$kode_user')";
        $res = $db->query($sql);
        if($res)
        {
            if($db->affected_rows>0)
            {
                header("Location: penggajian.php?success=1");
            }
        }
        else
